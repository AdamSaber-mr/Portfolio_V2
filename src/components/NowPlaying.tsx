...
import {
  FEATURED_TRACK, TRACKS_RECENT, TRACKS_TOP, spotifySearchUrl,
  type Track, type Strings,
} from '../data';
...
export default function NowPlaying({ s }: { s: Strings }) {
  ...
  const list = tab === 'recent' ? TRACKS_RECENT : TRACKS_TOP;
  const open = (t: Track) => window.open(spotifySearchUrl(t), '_blank', 'noopener,noreferrer');

  const f = FEATURED_TRACK;
  ...
  return (
    <div className="page-pad" style={sx('max-width:1440px; margin:0 auto; padding:24px 56px 40px;')}>
      ...
      <div className="np-grid" data-reveal="" style={sx('display:grid; grid-template-columns:1.25fr 1fr; gap:20px; align-items:stretch;')}>
        ...
        {/* featured track */}
        <div style={sx(`position:relative; background:${f.color}; border-radius:22px; padding:26px; display:flex; flex-direction:column; overflow:hidden; color:#fff;`)}>
          ...
          <div style={sx('width:62%; max-width:230px; aspect-ratio:1/1; margin:14px auto 8px;')}>
            <Art track={f} radius={14} />
          </div>
          <div style={sx('margin-top:auto;')}>
            <h3 style={sx("font-family:'Space Grotesk',sans-serif; font-size:clamp(24px,3vw,32px); font-weight:700; letter-spacing:-.01em; line-height:1.05;")}>{f.title}</h3>
            <p style={sx('font-size:15px; margin-top:4px; color:rgba(255,255,255,.7);')}>{f.artist}</p>
            <span
              {...clickable(() => open(f))}
            ...
            <div style={sx('display:flex; align-items:center; gap:14px; margin-top:18px;')}>
              <div style={sx('flex:1; height:4px; border-radius:3px; background:rgba(255,255,255,.22); overflow:hidden;')}>
                <div style={sx('width:34%; height:100%; background:rgba(255,255,255,.85);')}></div>
              </div>
              <span style={sx("font-family:'JetBrains Mono',monospace; font-size:12px; color:rgba(255,255,255,.8);")}>{f.dur}</span>
              <span
                {...clickable(() => open(f), `Play ${f.title}`)}
                className="np-play"
        ...
        <div style={sx('display:flex; flex-direction:column; gap:12px;')}>
          {list.map((t, i) => (
            <div
              key={`${tab}-${i}`}
              {...clickable(() => open(t), `${t.title} — ${t.artist}`)}
              className="np-row"
            ...
            >
              ...
              <div style={sx('flex:1; min-width:0;')}>
                ...
                <div style={sx('display:flex; align-items:center; gap:9px; margin-top:8px;')}>
                  <div style={sx('flex:1; height:3px; border-radius:2px; background:rgba(255,255,255,.2);')}>
                    <div style={sx('width:28%; height:100%; border-radius:2px; background:rgba(255,255,255,.6);')}></div>
                  </div>
                  <span style={sx("font-family:'JetBrains Mono',monospace; font-size:11.5px; color:rgba(255,255,255,.7);")}>{t.dur}</span>
                </div>
              </div>
              <span style={sx('display:inline-flex; align-items:center; justify-content:center; width:30px; height:30px; border-radius:50%; border:1.5px solid rgba(255,255,255,.55); color:rgba(255,255,255,.85); flex:none;')}><PlusIcon size={15} /></span>
              <span className="np-play" style={sx('display:inline-flex; align-items:center; justify-content:center; width:40px; height:40px; border-radius:50%; background:#fff; color:#111; flex:none;')}><PlayIcon size={15} /></span>
            </div>
          ))}
        </div>
