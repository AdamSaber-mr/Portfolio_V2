  ...
  return (
    <div style={sx('position:sticky; top:0; z-index:100; backdrop-filter:blur(20px) saturate(160%); -webkit-backdrop-filter:blur(20px) saturate(160%); background:var(--navbg); border-bottom:1px solid var(--line);')}>
      <div className="nav-inner" style={sx('max-width:1440px; margin:0 auto; padding:18px 56px; display:flex; align-items:center; justify-content:space-between; gap:16px;')}>
        <div onClick={() => go('home')} style={sx('cursor:pointer; display:flex; align-items:center; gap:11px;')}>
        ...
        <div className="nav-links" style={sx('display:flex; gap:26px; align-items:center; font-size:14px; font-weight:500;')}>
          <span className="navlink" style={sx(navStyle(page === 'home'))} onClick={() => go('home')}>{s.navHome}</span>
          <span className="navlink" style={sx(navStyle(page === 'work'))} onClick={() => go('work')}>{s.navWork}</span>
          <span className="navlink" style={sx(navStyle(page === 'about'))} onClick={() => go('about')}>{s.navAbout}</span>
          <span className="navlink" style={sx(navStyle(page === 'contact'))} onClick={() => go('contact')}>{s.navContact}</span>
          <div style={sx('display:flex; align-items:center; gap:8px; padding-left:6px;')}>
            <span className="icon-btn" onClick={toggleLang} style={sx("font-family:'JetBrains Mono',monospace; font-size:11px; font-weight:500; color:var(--accent); border:1px solid var(--line); border-radius:30px; padding:6px 11px;")}>
              {langLabel}
            </span>
            <span className="icon-btn" onClick={toggleTheme} title="Thema" style={sx('display:inline-flex; align-items:center; justify-content:center; width:34px; height:34px; background:var(--surface); border:1px solid var(--line); border-radius:11px; color:var(--ink);')}>
              {isDark ? (
            ...
            <span className="btn nav-cv" onClick={() => go('contact')} style={sx('display:inline-flex; align-items:center; gap:7px; background:var(--surface); color:var(--ink); border:1px solid var(--line); padding:8px 15px; border-radius:11px; font-size:13px; font-weight:600; white-space:nowrap;')}>
