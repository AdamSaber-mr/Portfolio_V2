              ...
              {vis.map((c, i) => {
                ...
                return (
                  <div key={i} className="work3d-card" onClick={() => setSlide(i)} style={sx(style)}>
                    <div style={sx(`width:100%; height:100%; border-radius:20px; overflow:hidden; background:${c.color}; border:1px solid rgba(255,255,255,.08); box-shadow:0 36px 70px -24px rgba(0,0,0,.65); display:flex; flex-direction:column; justify-content:space-between;`)}>
                      <div style={sx('padding:16px;')}>
                        <span style={sx("display:inline-block; font-family:'JetBrains Mono',monospace; font-size:10.5px; color:#fff; background:rgba(0,0,0,.32); padding:5px 11px; border-radius:30px; backdrop-filter:blur(4px);")}>{c.kind}</span>
                      </div>
                      <div style={sx('padding:20px 18px 22px; background:linear-gradient(to top, rgba(0,0,0,.6), rgba(0,0,0,0));')}>
