        ...
        <div className="featured-cols" style={sx('columns:3; column-gap:20px;')}>
          {featured.map((p, i) => (
            <div key={i} data-reveal="" className="card3d" onClick={() => go('work')} style={sx('break-inside:avoid; margin-bottom:20px; background:var(--surface); border:1px solid var(--line); border-radius:14px; overflow:hidden; cursor:pointer;')}>
              <div style={sx(`aspect-ratio:${p.ratio}; background:${p.color}; position:relative;`)}>
                <span style={sx("position:absolute; left:14px; top:13px; font-family:'JetBrains Mono',monospace; font-size:10.5px; color:rgba(255,255,255,.9); background:rgba(0,0,0,.3); padding:4px 10px; border-radius:30px;")}>{p.kind}</span>
              </div>
